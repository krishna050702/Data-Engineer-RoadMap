...
|---------|----------|
| Inner Join | Returns only the rows matching value from both the tables. |
| Left Join | Returns all the rows from left table and matching value from the right table. |
| Right Join | Returns all the rows from right table and matching values from the left table. |
| Full Outer Join | Returns all rows when there is a match in either the left or the right table. If there is no macth, NULL values are returned for columns from the table without a match. |

<hr>

### Date Function
- ```CURRENT_DATE``` , ```CURRENT_TIME``` and ```CURRENT TIMESTAMP``` to return current data, time and timestamp.
...
- ```INTERVAL``` to add or subtract time interval in calculations.
...
- ```DATE_TRUNC()``` rounds down a date or timestamp to s pecified unit of time. It trims the finer details and retains the specified unit.
- ```truncated_to_month``` - It rounds down the date to the beginning of the month.
- ```truncated_to_day()``` - It rounds down the date to the beginning of the day.
- ```truncated_to_hour``` - It rounds down the time to the beginning of the hour.
- ```INTERVAL``` function is used to handle date and time gaps by adding and subtracting intervals.
<hr>


!!! Let's continue to Advanced SQL concepts.